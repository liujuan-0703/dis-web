...
var name=$(_a36).attr("textboxName")||"";
    //modify by xuelei.wang 2017-11-5
...
return jq.each(function(){
_a2c(this,text);
});//modify by xuelei.wang 2017-10-15
}, beforeSetText:function(){}, customAfterCheck:function(){},beforeHidePanel:function(node){ return true;},getValues:function(jq){
return _a30(jq[0]);
...
tree.tree("doFilter","");
    ...
    //modify by xuelei.wang 2018-1-31
    if(opts.multiple&&opts.enableToolButton) {
        setButtons(_abf);
        bindEvents(_abf);
    }
opts.onShowPanel.call(this);
...
$(this).tree(node.checked?"uncheck":"check",node.target);
    ...
    //modify by xuelei.wang 2017-10-17
if($(_abf).combo("beforeHidePanel",node)){
...
opts.onCheck.call(this,node,_ac3);
    ...
    function setButtons(target) {
        if(target==undefined||target.className==''||target.className=='tree')return ;
        var panel = $(target).combo('panel');
        if (panel.siblings('div.easy-ui-combo-tree-btn').length) {
            return
        }
        var button = $('<div class="easy-ui-combo-tree-btn" style="margin-bottom:0px;"><table cellspacing="0" cellpadding="0" style="width:100%"><tr></tr></table></div>').insertAfter(panel);
        var tr = button.find('tr');
        var _ac0 = $.data(target, "combotree");
        var opts = _ac0.options;
        for (var i = 0; i < opts.buttons.length; i++) {
            var td;
            if(i%2==0){
                td = $('<td style="border-right:1px solid #ddd;"></td>').appendTo(tr);
            }
            else{
                td = $('<td></td>').appendTo(tr);
            }
            var btn = opts.buttons[i];
            var t = $('<a class="easy-ui-combo-tree-btn-a" href="javascript:;"></a>').html($.isFunction(btn.text) ? btn.text(target) : btn.text).appendTo(td);
            t.attr('combotree-button-index', i);
        }
    ...
    function bindEvents(target) {
        ...
        var panel = $(target).combo('panel');
        panel.siblings("div.easy-ui-combo-tree-btn").unbind('.easy-ui-combo-tree-btn').bind('click.easy-ui-combo-tree-btn', function (e) {
            if ($(e.target).hasClass('easy-ui-combo-tree-btn-a')) {
                var index = parseInt($(e.target).attr('combotree-button-index'));
...
if($(_ac8).combo("getText")!=s){
    //modify by xuelei.wang 2017-10-15
    var textArr = s.split($.fn.combo.defaults.separator);
    if (textArr.indexOf('全部') != -1)
        textArr.splice(textArr.indexOf('全部'), 1);
    if (textArr.indexOf('全选') != -1)
        textArr.splice(textArr.indexOf('全选'), 1);
    s = textArr.join($.fn.combo.defaults.separator);
    s= $(_ac8).combo("beforeSetText",s);
     $(_ac8).combo("setText",s);
    //modify by xuelei.wang 2017-11-30
    $(_ac8).combo('textbox').attr('title',s);
}
}
$(_ac8).combo("setValues",vv);
//modify by xuelei.wang 2017-11-5
$(_ac8).combo("customAfterCheck",vv);
    //防止下拉选择控件"全选"时值提交到后台;
    var hiddenNode = $("[type=hidden][name='" + _ac8.id + "'][value='-1']");
...
$.fn.combotree.defaults=$.extend({},$.fn.combo.defaults,$.fn.tree.defaults,{editable:false,textField:null,
        //modify by xuelei.wang 2018-1-31
        ...
        buttons: [{
            ...
            handler: function (target) {
                ...
                var roots = treeView.tree('getRoots');
                for (var i = 0; i < roots.length; i++) {
                    treeView.tree('uncheck', roots[i].target);
                }
                var opts = $.data(target, "combotree").options;
                opts.onCheck.call(this);
            }
        }]
    ,unselectedValues:[],mappingRows:[],keyHandler:{up:function(e){
},down:function(e){
},left:function(e){
},right:function(e){
},enter:function(e){
